...
st.caption("Filtrér, redigér og find forekomster hurtigt. Navigation/related tælles ikke med i forekomster.")

db.init_db()

# Valgfri auto-import fra fil (når DB er tom)
...
with st.sidebar:
    ...
    # Import (upsert kun kw/hits/total – bevar status/notes/assigned)
    ...
    if st.button("🚀 Crawl hele domænet", type="secondary"):
        if not kw_final:
            ...
            db.init_db()
            prog = st.progress(0.0, text="Starter crawler…")
            rows_buf, BATCH, db_errors = [], 150, 0

            def on_progress(done: int, queued: int):
                pct = min(0.99, done / 8000)
                prog.progress(pct, text=f"Crawler… {done} sider · kø: {queued}")
...
with tab_review:
    ...
    if review_df.empty:
        ...
        res = st.multiselect("Markér som Done", options=list(review_df["url"]))
...
with tab_focus:
    ...
    if ga_top is None or len(ga_top) == 0:
        ...
        st.caption(f"GA i alt: {len(focus)} · Done i GA: {done_in_ga} · Viser: {len(df_show)}")
